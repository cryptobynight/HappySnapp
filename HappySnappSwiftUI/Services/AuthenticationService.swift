...
class AuthenticationService: ObservableObject {
    ...
    @LazyInjected private var favouritesRepository: FavouritesRepository
    ...
    func signIn() {
        if Auth.auth().currentUser == nil {
            ...
            Auth.auth().signInAnonymously() { [self] authResult, error in
                ...
                for task in initialCategories {
                    self.taskRepository.addTask(task)
    ...
    private func registerStateListener() {
        ...
        self.handle = Auth.auth().addStateDidChangeListener { (auth, user) in
            ...
            if let user = user {
                ...
                print("User signed out...creating new anonymous user.")
